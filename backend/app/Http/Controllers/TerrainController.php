...
class TerrainController extends Controller
{
    ...
    {
        $query = Terrain::query()->where('status', 'active')->with('slots');
    ...
    {
        $query = Terrain::query()->with(['slots', 'owner:id,name,email'])->latest();
        $this->applyFilters($query, $request);
    ...
    public function store(Request $request)
    {
        ...
        return response()->json($terrain->load(['slots', 'owner:id,name,email']), 201);
    }
    ...
    public function update(Request $request, Terrain $terrain)
    {
        ...
        $terrain->update($data);
        return response()->json($terrain->fresh()->load(['slots', 'owner:id,name,email']));
    }
    ...
    {
        if ($request->filled('city')) $query->where('city', $request->string('city')->toString());
        if ($request->filled('quartier')) $query->where('quartier', $request->string('quartier')->toString());
    ...
    private function uniqueSlug(string $name, ?int $ignoreId = null): string
    {
        ...
        $slug = $base;
        $counter = 2;
        while (Terrain::query()->where('slug', $slug)
